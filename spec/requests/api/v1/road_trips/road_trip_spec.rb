...
RSpec.describe 'Road_Trip Endpoint -', type: :request do
  ...
  it 'creates road trips' do
    post '/api/v1/road_trip', params:
      ...
      expect(output[:data][:attributes][:origin]).to eq('Denver, CO')
      expect(output[:data][:attributes][:destination]).to eq(RoadTrip.last.destination)
      expect(output[:data][:attributes][:destination]).to eq('Pueblo, CO')
  ...
  it 'handles requests with missing info' do
    post '/api/v1/road_trip', params:
    {
      "origin": "Denver, CO",
      "api_key": "#{@user.api_key}"
    ...
    expect(output[:data][:attributes][:error]).to eq('Please provide an origin and a destination.')
  end
