...
RSpec.describe 'Forecast Endpoint -', type: :request do
  ...
  it 'provides current_details for a city' do
    ...
    expect(@expected["data"]["attributes"]["current_details"]["uv_index_interpreted"]).to eq('low')
  ...
  it 'provides daily forecast for the next 5 days' do
    ...
    expect(@expected["data"]["attributes"]["daily_forecast"]["attributes"][0]["image"]).to eq("http://openweathermap.org/img/wn/#{@weather[:daily][0][:weather][0][:icon]}@2x.png")
    ...
    expect(@expected["data"]["attributes"]["daily_forecast"]["attributes"][4]["image"]).to eq("http://openweathermap.org/img/wn/#{@weather[:daily][4][:weather][0][:icon]}@2x.png")
