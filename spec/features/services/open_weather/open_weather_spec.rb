...
RSpec.describe 'Open Weather Service: ', type: :feature do
  describe 'As a user when' do
    it 'I a lat/long I get back the weather' do
      ...
      stub_request(:get, "https://api.openweathermap.org/data/2.5/onecall?lat=coordinates[:lat]&lon=coordinates[:lng]&exclude=minutely&appid=#{ENV['OPEN_WEATHER_KEY']}")
                  .to_return(status: 200, body: json_response, headers: {})

      openweather = OpenWeatherService.find_weather(coordinates)
      expect(openweather[:current][:sunrise]).to eq(1591437208)
      expect(openweather[:current][:weather][:description]).to eq('few clouds')
      expect(openweather[:hourly][:humidity]).to eq(68)
      expect(openweather[:hourly][:weather][:description]).to eq('few clouds')
      expect(openweather[:daily][7][:sunset]).to eq(1592096494)
      expect(openweather[:daily][7][:weather][:description]).to eq('heavy intensity rain')
    end
  end
end
