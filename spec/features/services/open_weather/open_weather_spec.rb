...
RSpec.describe 'Open Weather Service: ', type: :feature do
  describe 'As a user when' do
    it 'I a lat/long I get back the weather' do
      ...
      expect(openweather[:current][:weather][0][:description]).to eq('clear sky')
      expect(openweather[:hourly][0][:humidity]).to eq(60)
      expect(openweather[:hourly][0][:weather][0][:description]).to eq('clear sky')
